...
def test_create_api_key_and_use_public_api():
    os.environ["TESTING"] = "1"
    ...
    db.close()
    resp = client.post(
        f"/orgs/{org_id}/api-keys", json={"name": "test", "scopes": ["projects:read"]}
    )
    assert resp.status_code == 200
    key = resp.json()["key"]
    # call public API
    resp = client.get("/api/v1/projects", headers={"Authorization": f"Bearer {key}"})
    assert resp.status_code == 200
